void main() {
    ...
    // Rescale the position to pick a nice looking part of the set
    //z *= vec2(2.0,2.0);
    z -= vec2(0,0.7);

    //vec2 c = z;
    //vec2 c = vec2(-0.7,0.4);    // static image
    //vec2 c = vec2(-0.7 + cos(u_time) / 31.0,0.4 + sin(u_time) / 33.0);   // changes with passing time
    vec2 c = vec2(-0.5 + cos(u_time) / 20.0, 0.5 + sin(u_time)/20 );   // larger denominators slows things down
